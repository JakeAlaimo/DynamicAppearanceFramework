...
public:
    ...
    void AddTransformationCurve(TransformationCurve curve, std::string actorOverrideName);

private:
    /// <summary>The type of transformation this group manages</summary>
    TraitType m_transformationType = TraitType::None;

    /// <summary>Collection of transformation curves under this group, indexed by actor name (for per-actor overrides)</summary>
    std::vector<std::map<std::string, TransformationCurve>> m_transformationCurves;

    // ^ this structure may not make sense
